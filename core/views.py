...
def exchange(request):
    ...
    return render(request, 'core/transaction.html',{'accounts':accounts,'arrows':arrows,'event_counters':event_counters,'events':events,'txns':txns,'registrations':registrations,'transfers':transfers,'arrow_updates':arrow_updates,'balance_updates':balance_updates})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            with transaction.atomic():
                ...
                new_txn = Txn.objects.create(id=txn_number,event=new_event,txn_previous_hash=last_txn.txn_hash,txn_type='Commitment',sender=sender,sender_seq_no=sender_seq_no,txn_message=message_string,signature=signature,txn_data=txn_data,txn_hash=txn_hash)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            with transaction.atomic():
                ...
                new_event = Event.objects.create(id=event_counter.last_event_no+1,timestamp=current_time, event_type='Txn') #handle integrity error for create
                new_txn = Txn.objects.create(id=txn_number,event=new_event,txn_previous_hash=last_txn.txn_hash,txn_type='Revelation',sender=sender,sender_seq_no=sender_seq_no,txn_message=message_string,signature=signature,txn_data=txn_data,txn_hash=txn_hash)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            with transaction.atomic():
                ...
                new_event = Event.objects.create(id=event_counter.last_event_no+1,timestamp=current_time, event_type='Txn') #handle integrity error for create
                new_txn = Txn.objects.create(id=txn_number,event=new_event,txn_previous_hash=last_txn.txn_hash,txn_type='Transfer',sender=sender,sender_seq_no=sender_seq_no,txn_message=message_string,signature=signature,txn_data=txn_data,txn_hash=txn_hash)
...
def register(request):
    if request.method == 'POST':
        ...
        if userform.is_valid() and form.is_valid():
            ...
            #     new_txn = Txn.objects.create(id=1,event=new_event,txn_previous_hash='Genesis',txn_type='Register',sender=sender,sender_seq_no=sender_seq_no,txn_message=message_string,signature=signature,txn_data=txn_data,txn_hash=txn_hash)
            ...
            with transaction.atomic():
                ...
                new_event = Event.objects.create(id=event_counter.last_event_no+1,timestamp=current_time, event_type='Txn') #handle integrity error for create
                new_txn = Txn.objects.create(id=txn_number,event=new_event,txn_previous_hash=last_txn.txn_hash,txn_type='Register',sender=sender,sender_seq_no=sender_seq_no,txn_message=message_string,signature=signature,txn_data=txn_data,txn_hash=txn_hash)
                new_registration = Registration.objects.create(txn=new_txn,name=name,photo_hash=photo_hash)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            with transaction.atomic():
                ...
                last_txn = Txn.objects.get(id = txn_counter.last_txn_no)
                
                current_time = timezone.now()
                if current_time < last_txn.txn_created:
                    messages.error(request, 'Transaction was unsuccessful. Wrong datetime.')  #should really never happen
                ...
                if arrow_status == 'Trust':
                    new_status = 1
                elif arrow_status == 'Distrust':
                    new_status = -1
                else:
                    new_status = 0

                if arrow.status == new_status:
                    messages.warning(request, 'Nothing changed as your vote was already set to '+arrow.get_arrow_status_display()+'.')
                    return redirect('/changevote/')   


                txn_number = txn_counter.last_txn_no + 1
                message_string = 'Type:ChangeVote,Sender:'+sender_pk+',SeqNo:'+str(sender_seq_no)+',Target:'+target_pk+',Vote:'+arrow_status
                txn_data = '"TxnNo":"'+str(txn_number)+'","Created":"'+str(current_time.strftime("%Y-%m-%d-%H:%M:%S"))+'","Message":"'+message_string+'","Signature":"'+signature+'","PreviousHash":"'+last_txn.txn_hash+'"'
                txn_hash  = nacl.hash.sha512(txn_data.encode('utf-8'), encoder=nacl.encoding.HexEncoder)
                new_transaction = Txn(txn_previous_hash=last_txn.txn_hash,txn_created=current_time,txn_type='ChangeVote',sender=sender,sender_seq_no=sender_seq_no,signature=signature,txn_hash=txn_hash,recipient=target,vote=arrow_status,txn_data=txn_data,txn_message=message_string)
